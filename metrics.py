...
def ft_int_gpu(N, y, dist_metric="euclidean"):
    """
    GPU-accelerated INT Index computation using CuPy and RAPIDS cuML.
    ...
    for id_cls_a, id_cls_b in combinations(range(class_num), 2):  # Use itertools.combinations
        N_gpu
...
def precompute_concept_dist_gpu(
    N: np.ndarray, concept_dist_metric: str = "euclidean"
) -> cp.ndarray:
    """
    Precompute the distance matrix for the dataset using cuML and CuPy.
    ...
    concept_dist_metric : str, optional
        Metric used to compute distances. Only 'euclidean' is supported.
    ...
    """
    if concept_dist_metric != "euclidean":
        raise ValueError("Only 'euclidean' distance is supported in this GPU-optimized version.")
...
def ft_conceptvar_gpu(
    ...
    concept_distances: t.Optional[cp.ndarray] = None,
) -> t.Tuple[float, cp.ndarray]:
    """
    Compute the concept variation that estimates the variability of
    class labels among examples using CuPy and cuML for GPU acceleration.
    ...
    conceptvar_alpha : float, optional
        The alpha value to adjust the weight. Higher alpha reduces weight impact.
    concept_dist_metric : str, optional
        Metric used to compute distances. Defaults to 'euclidean'.
    concept_minimum : float, optional
        Minimum value to avoid division by zero. Defaults to 1e-10.
    concept_distances : cp.ndarray, optional
        Precomputed distance matrix. If None, it will be computed.

    Returns
    -------
    Tuple[float, cp.ndarray]
        Standard deviation of concept variation and array containing the concept variation for each example.
    """
    ...
    n_col = N.shape[1]
    concept_distances = np.asarray(concept_distances.get())
    div = np.sqrt(np.array(n_col)) - concept_distances
    div = np.clip(div, a_min=concept_minimum, a_max=None)  # Avoid division by zero
    ...
    conceptvar_by_example = cp.sum(weights * class_diff, axis=0) / cp.sum(weights, axis=0)
...
def ft_wg_dist_gpu(
    ...
    wg_dist_example = cp.sum(weights * concept_distances, axis=0) / np.sum(
        weights, axis=0
    )

    # The original meta-feature is the mean of the return.
    # It will be done by summary functions.
    std_dev = cp.std(wg_dist_example).item()
